...
async function injectOrShowConfigCode(
  injectFn: () => Promise<boolean>,
  {
    code,
    filePath,
    fallbackFileMessage,
...
async function injectAndShowCypressJsonConfig(cypressJsonPath: string, componentFolder: string) {
...
async function injectAndShowPluginConfig<T>(
  template: Template<T>,
  {
    templatePayload,
    pluginsFilePath,
  ...
  const ast = template.getPluginsCodeAst(templatePayload, {
    cypressProjectRoot,
  })
...
export async function initComponentTesting<T>({ config, useYarn, cypressConfigPath }: InitComponentTestingOptions) {
  ...
  const framework = await installFrameworkAdapter(cypressProjectRoot, {
    useYarn,
  })
  const { possibleTemplates, defaultTemplate, defaultTemplateName, templatePayload } = await guessTemplate<T>(
    framework,
    cypressProjectRoot
  )

  const pluginsFilePath = path.resolve(cypressProjectRoot, config.pluginsFile ?? './cypress/plugins/index.js')

  const templateChoices = Object.keys(possibleTemplates).sort((key) => {
    return key === defaultTemplateName ? -1 : 0
  })

  const { chosenTemplateName, componentFolder }: Record<string, string> = await inquirer.prompt([
    {
      type: 'list',
    ...
    },
    {
      ...
      validate: (input) => {
        return input === '' || !/^[a-zA-Z].*/.test(input) ? `Directory "${input}" is invalid` : true
      },
  ...
  console.log(
    `Find examples of component tests for ${chalk.green(chosenTemplateName)} in ${chalk.underline(
