...
export function extractRollupConfigPathFromScript(script: string) {
  if (script.includes('rollup ')) {
    ...
    const configArgIndex = cliArgs.findIndex((arg) => arg === '--config' || arg === '-c')
...
export const RollupTemplate: Template<{ rollupConfigPath: string }> = {
  message: 'It looks like you have custom `rollup.config.js`. We can use it to bundle components for testing.',
  getExampleUrl: () => {
  ...
  getPluginsCodeAst: (payload, { cypressProjectRoot }) => {
    ...
    const rollupConfigPath = payload ? path.relative(cypressProjectRoot, payload.rollupConfigPath) : 'rollup.config.js'

    return {
      RequireAst: babel.template.ast(
        ['const path = require("path")', 'const { startDevServer } = require("@cypress/rollup-dev-server")'].join('\n')
      ),
      IfComponentTestingPluginsAst: babel.template.ast(
        [
          ...
          `    options,`,
          includeWarnComment ? ' // TODO replace with valid rollup config path' : '',
  ...
  test: (root) => {
    ...
    return packageJsonIterator.map(({ scripts }, packageJsonPath) => {
      ...
      for (const script of Object.values(scripts)) {
        const rollupConfigRelativePath = extractRollupConfigPathFromScript(script)

        if (rollupConfigRelativePath) {
          const directoryRoot = path.resolve(packageJsonPath, '..')
          const rollupConfigPath = path.resolve(directoryRoot, rollupConfigRelativePath)
