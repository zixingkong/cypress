...
export default class CypressCTOptionsPlugin implements Plugin {
  ...
  private setupCustomHMR = (compiler: webpack.Compiler) => {
    compiler.hooks.afterCompile.tap('CypressCTOptionsPlugin', (compilation: compilation.Compilation) => {
    ...
    compiler.hooks.afterEmit.tap('CypressCTOptionsPlugin', (compilation: compilation.Compilation) => {
  ...
  private plugin = (compilation: compilation.Compilation) => {
    this.devServerEvents.on('dev-server:specs:changed', (specs) => {
      ...
      const utimesSync: UtimesSync = semver.gt('4.0.0', webpack.version)
        ? inputFileSystem.fileSystem.utimesSync
        : fs.utimesSync
