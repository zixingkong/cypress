  ...
  <div class="pizza-order">
    ...
    <div class="toppings" style="float: left; width: 50%">
      ...
      <ul>
        <li v-for="topping in toppings" :key="topping.name">
          <input type="checkbox" :id="topping.name" :value="topping" v-model="selected" />
          <label :for="topping.name"> {{ topping.icon }} {{ topping.name }} </label>
        </li>
      </ul>
    </div>
    <div class="order-overview" style="float: left; width: 50%">
      ...
      <ul v-if="selected.length">
        <li v-for="topping in selected" :key="topping.name">{{ topping.icon }} {{ topping.name }}</li>
      </ul>
      <div v-else>👈 Select your toppings.</div>
    </div>
  </div>
</template>
...
const filterByName = (list, name) => list.filter((i) => i.name === name)
