  ...
  context('#startXhrServer', () => {
    ...
    describe('url rewriting', () => {
      ...
      it('rewrites urls with nested segments', () => {
        cy.server()
          ...
          .then(function () {
            ...
            expect(this.open).to.be.calledWith(
              'GET',
              '/__cypress/xhrs/http://localhost:3500/fixtures/phones/phones.json'
            )
          })
      })
      ...
      it('can stub CORS string routes', () => {
        cy.server()
          .route('http://localhost:3501/fixtures/app.json')
          .as('getPhones')
          .window()
          .then(function (win) {
            this.open = cy.spy(cy.state('server').options, 'onOpen')
      ...
      it('decodes proxy urls #2', () => {
        cy.server()
          ...
          .then(function (win) {
            this.open = cy.spy(cy.state('server').options, 'onOpen')
            win.$.get(
              '/accounts?page=1&%24filter=(rowStatus+eq+1)&%24orderby=name+asc&includeOpenFoldersCount=true&includeStatusCount=true'
            )
          ...
          .then(function () {
            ...
            expect(xhr.url).to.eq(
              'http://localhost:3500/accounts?page=1&$filter=(rowStatus+eq+1)&$orderby=name+asc&includeOpenFoldersCount=true&includeStatusCount=true'
            )
    ...
    describe('request parsing', () => {
      ...
      it('provides the correct requestBody on multiple requests', () => {
        ...
        cy.server()
          ...
          .wait('@getFoo')
          .its('requestBody')
          .should('deep.eq', { foo: 'bar2' })
      })

      it('handles arraybuffer', () => {
        cy.server()
          .route('GET', /arraybuffer/)
          .as('getBuffer')
          .window()
          .then((win) => {
            const xhr = new win.XMLHttpRequest()
    ...
    describe('#issue #85', () => {
      it('correctly returns the right XHR alias', () => {
        cy.server()
          ...
          .wait('@getFolders')
          .wait('@getFoo')
          .route(/folders/, { foo: 'baz' })
          .as('getFoldersWithSearch')
          .window()
          .then((win) => {
      ...
      context('requests', () => {
        ...
        it('can access requestHeaders', () => {
          cy.server()
            ...
            .wait('@getFoo')
            .its('requestHeaders')
            .should('have.property', 'x-token', '123')
        })
      })
  ...
  context('#server', () => {
    it('logs deprecation warning', () => {
      ...
      cy.server().then(function () {
        expect(Cypress.utils.warning).to.be.calledWithMatch(
          /^`cy\.server\(\)` has been deprecated and will be moved to a plugin in a future release\. Consider migrating to using `cy\.intercept\(\)` instead\./
        )
      })
    })
  ...
  context('#route', () => {
    ...
    it('logs deprecation warning', () => {
      ...
      cy.route('*').then(function () {
        expect(Cypress.utils.warning).to.be.calledWithMatch(
          /^`cy\.route\(\)` has been deprecated and will be moved to a plugin in a future release\. Consider migrating to using `cy\.intercept\(\)` instead\./
        )
      })
    })
    ...
    it('can stub requests with uncommon HTTP methods', () => {
      cy.route('PROPFIND', '/foo', 'foo bar baz')
        .as('getFoo')
        .window()
        .then((win) => {
          win.$.ajax({
    ...
    it('does not warn if an invalid percent-encoded URL is used', () => {
      ...
      cy.route('GET', 'http://example.com/%E0%A4%A').then(() => {
    ...
    describe(
      ...
      () => {
        ...
        it('url must be a string or regexp', (done) => {
          cy.on('fail', (err) => {
            expect(err.message).to.include(
              '`cy.route()` was called with an invalid `url`. `url` must be either a string or regular expression.'
            )
        ...
        it('requires a url when given a response', (done) => {
          cy.on('fail', (err) => {
            expect(err.message).to.include(
              '`cy.route()` must be called with a `url`. It can be a string or regular expression.'
            )
        ...
        _.each([null, undefined], (val) => {
          it(`throws if response options was explicitly set to ${val}`, (done) => {
            cy.on('fail', (err) => {
              expect(err.message).to.include(
                '`cy.route()` cannot accept an `undefined` or `null` response. It must be set to something, even an empty string will work.'
              )
              expect(err.docsUrl).to.eq('https://on.cypress.io/route')
        ...
        it('requires arguments', (done) => {
          cy.on('fail', (err) => {
            expect(err.message).to.include(
              '`cy.route()` was not provided any arguments. You must provide valid arguments.'
            )
            expect(err.docsUrl).to.eq('https://on.cypress.io/route')
        ...
        it.skip('explodes if response fixture signature errors', function (done) {
          this.trigger = cy
            .stub(this.Cypress, 'trigger')
            .withArgs('fixture')
            .callsArgWithAsync(2, { __error: 'some error' })
        ...
        it.skip('does not retry (cancels existing promise) when xhr errors', (done) => {
          ...
          cy.route({
            ...
            .get('button')
            .should('have.class', 'does-not-exist')
        })
        ...
        it('explodes if response alias cannot be found', function (done) {
          cy.on('fail', (err) => {
            ...
            expect(err.message).to.eq(
              '`cy.route()` could not find a registered alias for: `@bar`.\nAvailable aliases are: `foo`.'
            )
            expect(lastLog.get('name')).to.eq('route')
            expect(lastLog.get('error')).to.eq(err)
          ...
          cy.wrap({ foo: 'bar' }).as('foo').route(/foo/, '@bar')
        })
  ...
  context('consoleProps logs', () => {
    ...
    describe('ignored routes', () => {
      ...
      it('ignores query params when filtering routes', () => {
        ...
        cy.wait('@getQueryParam').its('response.body').should('deep.equal', { foo: 'bar' })
      })
      ...
      it('ignores hashes when filtering routes', () => {
        ...
        cy.wait('@getHash').its('response.body').should('deep.equal', { foo: 'bar' })
      })
      ...
      it('overrides ignoring resources when passed as option', () => {
        ...
        cy.wait('@getJSResource').its('response.body').should('deep.equal', { foo: 'bar' })

        cy.wait('@getHTMLResource').its('response.body').should('deep.equal', '<html></html>')

        cy.wait('@getCSSResource').its('response.body').should('deep.equal', 'body: {color: red;}')
      })
    })
    ...
    describe('route setup', () => {
      ...
      it('sends back 404 when request doesnt match route', () => {
        cy.then(function () {
          ...
          expect(consoleProps.Note).to.eq(
