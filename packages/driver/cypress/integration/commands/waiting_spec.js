  ...
  context('#wait', () => {
    ...
    describe('alias argument', () => {
      ...
      it('passes timeout option down to requestTimeout of wait', (done) => {
        ...
        cy.server().route('GET', /.*/, {}).as('fetch').wait('@fetch', { timeout: 900 })
      })
      ...
      it('waits for requestTimeout override', (done) => {
        ...
        cy.server().route('GET', '*', {}).as('fetch').wait('@fetch', { requestTimeout: 199 })
      })

      it(
        'waits for responseTimeout',
        {
          responseTimeout: 299,
        },
        (done) => {
      ...
      describe(
        ...
        () => {
          ...
          it(
            'throws when route is never resolved',
            {
              requestTimeout: 100,
            },
            (done) => {
              cy.on('fail', (err) => {
                expect(err.message).to.include(
                  '`cy.wait()` timed out waiting `100ms` for the 1st request to the route: `fetch`. No request ever occurred.'
                )
          ...
          it(
            'throws when alias is never requested',
            {
              requestTimeout: 100,
            },
            (done) => {
          ...
          it('throws when 2nd alias doesnt match any registered alias', (done) => {
            cy.on('fail', (err) => {
              expect(err.message).to.eq(
                '`cy.wait()` could not find a registered alias for: `@bar`.\nAvailable aliases are: `foo`.'
              )
          ...
          it(
            'throws whenever an alias times out',
            {
              requestTimeout: 1000,
            },
            (done) => {
              cy.on('fail', (err) => {
                expect(err.message).to.include(
                  '`cy.wait()` timed out waiting `1000ms` for the 1st request to the route: `foo`. No request ever occurred.'
                )
              ...
              cy.server().route(/foo/, {}).as('foo').route(/bar/, {}).as('bar').wait(['@foo', '@bar'])
            }
          )

          it(
            'throws when bar cannot resolve',
            {
              requestTimeout: 100,
            },
            (done) => {
              cy.on('fail', (err) => {
                expect(err.message).to.include(
                  '`cy.wait()` timed out waiting `100ms` for the 1st request to the route: `bar`. No request ever occurred.'
                )
          ...
          it(
            'does not throw another timeout error when 2nd alias is missing @',
            {
              requestTimeout: 100,
            },
            (done) => {
          ...
          it(
            'does not throw again when 2nd alias doesnt reference a route',
            {
              requestTimeout: 100,
            },
            (done) => {
              ...
              cy.server().route(/foo/, {}).as('foo').get('body').as('bar').wait(['@foo', '@bar'])
            }
          )

          it(
            'does not retry after 1 alias times out',
            {
              requestTimeout: 1000,
            },
            (done) => {
          ...
          it(
            'throws waiting for the 3rd response',
            {
              requestTimeout: 200,
            },
            (done) => {
              ...
              cy.on('fail', (err) => {
                expect(err.message).to.include(
                  '`cy.wait()` timed out waiting `200ms` for the 3rd request to the route: `get.users`. No request ever occurred.'
                )
          ...
          it(
            'throws waiting for the 2nd response',
            {
              requestTimeout: 200,
            },
            (done) => {
              ...
              cy.on('fail', (err) => {
                expect(err.message).to.include(
                  '`cy.wait()` timed out waiting `200ms` for the 2nd request to the route: `getUsers`. No request ever occurred.'
                )
              ...
              cy.server().route(/users/, resp).as('getUsers').wait('@getUsers').wait('@getUsers')
            }
          )

          it(
            'throws waiting for the 2nd request',
            {
              requestTimeout: 200,
            },
            (done) => {
              ...
              cy.server().route(/users/, resp).as('getUsers').wait('@getUsers.request').wait('@getUsers.request')
            }
          )

          it(
            'throws when waiting for response to route',
            {
              responseTimeout: 100,
            },
            (done) => {
              cy.on('fail', (err) => {
                expect(err.message).to.include(
                  '`cy.wait()` timed out waiting `100ms` for the 1st response to the route: `response`. No response ever occurred.'
                )
          ...
          it(
            'throws when waiting for 2nd response to route',
            {
              responseTimeout: 200,
            },
            (done) => {
              cy.on('fail', (err) => {
                expect(err.message).to.include(
                  '`cy.wait()` timed out waiting `200ms` for the 2nd response to the route: `response`. No response ever occurred.'
                )
          ...
          it(
            'throws when waiting for 1st response to bar',
            {
              responseTimeout: 200,
            },
            (done) => {
              cy.on('fail', (err) => {
                expect(err.message).to.include(
                  '`cy.wait()` timed out waiting `200ms` for the 1st response to the route: `bar`. No response ever occurred.'
                )
          ...
          it(
            'throws when waiting on the 2nd request',
            {
              requestTimeout: 200,
            },
            (done) => {
          ...
          it(
            'throws when waiting on the 3rd request on array of aliases',
            {
              requestTimeout: 500,
            ...
            (done) => {
              ...
              cy.on('fail', (err) => {
                expect(err.message).to.include(
                  '`cy.wait()` timed out waiting `500ms` for the 1st request to the route: `get.three`. No request ever occurred.'
                )
          ...
          it(
            'throws when waiting on the 3rd response on array of aliases',
            {
              requestTimeout: 200,
            ...
            (done) => {
              ...
              cy.on('fail', (err) => {
                expect(err.message).to.include(
                  '`cy.wait()` timed out waiting `1000ms` for the 1st response to the route: `getThree`. No response ever occurred.'
                )
          ...
          it('throws when passed multiple string arguments', (done) => {
            cy.on('fail', (err) => {
              expect(err.message).to.eq(
                "`cy.wait()` was passed invalid arguments. You cannot pass multiple strings. If you're trying to wait for multiple routes, use an array."
              )
              expect(err.docsUrl).to.eq('https://on.cypress.io/wait')
          ...
          _.each(
            ...
            (attrs) => {
              it(`throws when 1st arg is ${attrs.type}`, (done) => {
                cy.on('fail', (err) => {
                  expect(err.message).to.eq(
                    `\`cy.wait()\` only accepts a number, an alias of a route, or an array of aliases of routes. You passed: \`${
                      attrs.errVal || JSON.stringify(attrs.val)
                    }\``
                  )
                  expect(err.docsUrl).to.eq('https://on.cypress.io/wait')
      ...
      describe('alias argument errors', () => {
        ...
        it('only logs once', function (done) {
          cy.on('fail', (err) => {
            ...
            expect(err.message).to.eq(
              '`cy.wait()` could not find a registered alias for: `@foo`.\nYou have not aliased anything yet.'
            )
        ...
        it(
          ...
          function (done) {
            cy.on('fail', (err) => {
              ...
              expect(err.message).to.include(
                '`cy.wait()` timed out waiting `100ms` for the 1st request to the route: `getBar`. No request ever occurred.'
              )
      ...
      describe('timeouts', function () {
        it(
          ...
          function (done) {
            ...
            cy.server().route('GET', '*', {}).as('fetch').wait('@fetch')
          }
        )
