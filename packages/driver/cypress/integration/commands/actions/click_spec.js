...
describe('src/cy/commands/actions/click', () => {
  ...
  context('#click', () => {
    ...
    it('each click gets a full command timeout', () => {
      ...
      cy.get('#three-buttons button')
        ...
        .then(() => {
          ...
          expect(
            firstCallOptions !== secondCallOptions,
            'Expected click retry options to be different object references between clicks'
          ).to.be.true
        })
    })
    ...
    it('places cursor at the end of [contenteditable]', () => {
      cy.get('[contenteditable]:first').invoke('html', '<div><br></div>').click().then(expectCaret(0))

      cy.get('[contenteditable]:first').invoke('html', 'foo').click().then(expectCaret(3))

      cy.get('[contenteditable]:first').invoke('html', '<div>foo</div>').click().then(expectCaret(3))
    ...
    describe('pointer-events:none', () => {
      beforeEach(function () {
        cy.$$(
          '<div id="ptr" style="position:absolute;width:200px;height:200px;background-color:#08c18d;">behind #ptrNone</div>'
        ).appendTo(cy.$$('#dom'))
        this.ptrNone = cy
          .$$(
            `<div id="ptrNone" style="position:absolute;width:400px;height:400px;background-color:salmon;pointer-events:none;opacity:0.4;text-align:right">#ptrNone</div>`
          )
          .appendTo(cy.$$('#dom'))
        cy.$$(
          '<div id="ptrNoneChild" style="position:absolute;top:50px;left:50px;width:200px;height:200px;background-color:red">#ptrNone > div</div>'
        ).appendTo(this.ptrNone)
      ...
      it('should error with message about pointer-events', function () {
        const onError = cy.stub().callsFake((err) => {
          ...
          expect(_.keys(consoleProps)).deep.eq(['Command', 'Tried to Click', 'But it has CSS', 'Error'])
    ...
    describe('actionability', () => {
      ...
      it('passes options.animationDistanceThreshold to cy.ensureElementIsNotAnimating', () => {
        ...
        cy.get('button:first')
          .click({ animationDistanceThreshold: 1000 })
          .then(() => {
    ...
    describe('relative coordinate arguments', () => {
      ...
      it('can specify x and/or y to be 0', () => {
        const $btn = $('<button id="click-button">button</button>').prependTo(cy.$$('body'))
        ...
        cy.on('log:changed', (log, attr) => {
          if (log.name === 'click' && attr._emittedAttrs.coords) {
            ...
            expect(coords.x)
              .to.equal(coords.left)
              .to.equal(position.left + args[0])
            expect(coords.y)
              .to.equal(coords.top)
              .to.equal(position.top + args[1])
          }
        })
    ...
    describe('mousedown', () => {
      ...
      it('still gives focus to the focusable element even when click is issued to child element', () => {
        ...
        cy.get('#button-covered-in-span').click().focused().should('have.id', 'button-covered-in-span')
      })
      ...
      () => {
        ...
        it('throws when element is fixed position and being covered', function (done) {
          ...
          cy.on('fail', (err) => {
            ...
            expect(err.message).to.include(
              "`<button#button-covered-in-span>` is not visible because it has CSS property: `position: fixed` and it's being covered"
            )
        ...
        it('throws when element animation exceeds timeout', (done) => {
          ...
          cy.on('fail', (err) => {
            ...
            expect(err.message).to.include(
              '`cy.click()` could not be issued because this element is currently animating:\n'
            )
    ...
    describe('.log', () => {
      ...
      it('#consoleProps groups skips mouse move events if no mouse move', () => {
        ...
        cy.getAll('btn', 'pointerdown mousedown pointerup mouseup click')
          .each(shouldBeCalledWithCount(2))
          .then(function () {
            const { logs } = this
  ...
  context('#dblclick', () => {
    ...
    it('serially dblclicks a collection of anchors to the top of the page', () => {
      ...
      $anchors.on('click', handleClick)
      cy.$$('div#dom').on('click', cy.stub().as('topClick')).on('dblclick', cy.stub().as('topDblclick'))
      ...
      it('snapshots after clicking', () => {
        cy.get('button:first')
          ...
          .then(function () {
            ...
            expect(lastLog.get('snapshots')[0]).to.containSubset({
              name: 'before',
            })
            expect(lastLog.get('snapshots')[1]).to.containSubset({
              name: 'after',
            })
          })
      })
  ...
  context('#rightclick', () => {
    ...
    it('can rightclick disabled with force', () => {
      const el = cy.$$('input:first').prop('disabled', true)
...
describe('mouse state', () => {
  describe('mouse/pointer events', () => {
    ...
    describe('mouseout', () => {
      it('can move mouse from a div to another div', () => {
        ...
        Cypress.Promise.delay(5000).then(() => {
  ...
  describe('user experience', () => {
    ...
    it('can print table of keys on dblclick', () => {
      ...
      cy.wrap(null, { timeout: 1000 }).should(() => {
        spyTableName.resetHistory()
        spyTableData.resetHistory()
