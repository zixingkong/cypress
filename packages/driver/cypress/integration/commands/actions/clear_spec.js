const { _, $ } = Cypress
const { attachKeyListeners, shouldBeCalledOnce, shouldNotBeCalled } = require('../../../support/utils')
...
describe('src/cy/commands/actions/type - #clear', () => {
  ...
  it('can force clear when hidden in input', () => {
    const input = cy.$$('input:first').val('foo').hide()

    attachKeyListeners({ input })
    cy.get('input:first').focus().clear({ force: true }).should('have.value', '')
  ...
  it('can force clear when hidden in textarea', () => {
    const textarea = cy.$$('textarea:first').val('foo').hide()

    attachKeyListeners({ textarea })
    cy.get('textarea:first').focus().clear({ force: true }).should('have.value', '')
