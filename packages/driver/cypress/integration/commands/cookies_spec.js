...
describe('src/cy/commands/cookies', () => {
  ...
  context('test:before:run:async', () => {
    it('clears cookies before each test run', () => {
      ...
      Cypress.emitThen('test:before:run:async', {}).then(() => {
        ...
        expect(Cypress.automation).to.be.calledWith('clear:cookies', [{ domain: 'localhost', name: 'foo' }])
      })
    })
  ...
  context('#getCookies', () => {
      ...
      () => {
        ...
        it('logs once on error', function (done) {
          ...
          cy.on('fail', () => {
            ...
            expect(lastLog.get('error').message).to.contain(
              `\`cy.getCookies()\` had an unexpected error reading cookies from ${Cypress.browser.displayName}.`
            )
    ...
    describe('.log', () => {
      beforeEach(function () {
        ...
        Cypress.automation.withArgs('get:cookies', { domain: 'localhost' }).resolves([
          {
            name: 'foo',
            value: 'bar',
            domain: 'localhost',
  ...
  context('#getCookie', () => {
    it('returns single cookie by name', () => {
      ...
      cy.getCookie('foo')
        ...
        .then(() => {
          expect(Cypress.automation).to.be.calledWith('get:cookie', {
            domain: 'localhost',
            name: 'foo',
          })
        })
      ...
      () => {
        ...
        it('logs once on error', function (done) {
          ...
          cy.on('fail', (err) => {
            ...
            expect(lastLog.get('error').message).to.contain(
              `\`cy.getCookie()\` had an unexpected error reading the requested cookie from ${Cypress.browser.displayName}.`
            )
            expect(lastLog.get('error').message).to.contain('some err message')
        ...
        it('requires a string name', function (done) {
          cy.on('fail', (err) => {
            ...
            expect(lastLog.get('error').message).to.eq(
              '`cy.setCookie()` must be passed two string arguments for `name` and `value`.'
            )
  ...
  context('#clearCookie', () => {
    it('returns null', () => {
      ...
      cy.clearCookie('foo')
        .should('be.null')
        .then(() => {
          expect(Cypress.automation).to.be.calledWith('clear:cookie', {
            domain: 'localhost',
            name: 'foo',
          })
        })
      ...
      () => {
        ...
        it('logs once on error', function (done) {
          ...
          cy.on('fail', (err) => {
            ...
            expect(this.logs.length).to.eq(1)
            expect(lastLog.get('error').message).to.contain(
              `\`cy.clearCookie()\` had an unexpected error clearing the requested cookie in ${Cypress.browser.displayName}.`
            )
            expect(lastLog.get('error').message).to.contain('some err message')
    ...
    describe('.log', () => {
      ...
      it('#consoleProps', () => {
        cy.clearCookie('foo').then(function (cookie) {
          ...
          expect(c['Yielded']).to.eq('null')
          expect(c['Cleared Cookie']).to.deep.eq({
            name: 'foo',
            value: 'bar',
            domain: 'localhost',
            path: '/',
  ...
  context('#clearCookies', () => {
    ...
    describe('timeout', () => {
      beforeEach(() => {
        Cypress.automation.withArgs('get:cookies').resolves([{}]).withArgs('clear:cookies').resolves({})
      })
      ...
      () => {
        ...
        it("logs once on 'get:cookies' error", function (done) {
          ...
          cy.on('fail', (err) => {
            ...
            expect(this.logs.length).to.eq(1)
            expect(lastLog.get('error').message).to.contain(
              `\`cy.clearCookies()\` had an unexpected error clearing cookies in ${Cypress.browser.displayName}.`
            )
            expect(lastLog.get('error').message).to.contain('some err message')
            expect(lastLog.get('error')).to.eq(err)
        ...
        it("logs once on 'clear:cookies' error", function (done) {
          Cypress.automation.withArgs('get:cookies').resolves([{ name: 'foo' }, { name: 'bar' }])
    ...
    it('throws error on use of renamed whitelist option', (done) => {
      cy.on('fail', (err) => {
        expect(err.message).to.include(
          '`Cypress.Cookies.defaults` `whitelist` option has been renamed to `preserve`. Please rename `whitelist` to `preserve`.'
        )
