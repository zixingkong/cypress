...
describe('src/cy/commands/navigation', () => {
  context('#reload', () => {
    ...
    // TODO: fix this
    it.skip(
      '(FLAKY) sets timeout to Cypress.config(pageLoadTimeout)',
      {
    ...
    describe(
      ...
      () => {
        ...
        it(
          'throws passing more than 2 args',
          {
    ...
    describe(
      ...
      () => {
        ...
        it('only logs once on error', function (done) {
          ...
          cy.visit('/fixtures/jquery.html').go('back', { timeout: 1 })
        })
      }
    )
  ...
  context('#visit', () => {
    ...
    it('calls resolve:url with http:// when localhost', () => {
      ...
      cy.visit('localhost:3500/timeout').then(() => {
    ...
    it('can visit relative pages on the same originPolicy', () => {
      ...
      cy.visit('http://localhost:3500/fixtures/dimensions.html').visit('/fixtures/jquery.html')
    })

    it('can visit relative pages with domain like query params', () => {
      cy.visit('http://localhost:3500/fixtures/generic.html').visit(
        'http://localhost:3500/fixtures/dimensions.html?email=briancypress.io'
      )
    })

    it('can visit pages with non-2xx status codes when option failOnStatusCode is false', () => {
      cy.visit('localhost:3500/status-404', {
    ...
    it('strips username + password out of the url when provided', () => {
      ...
      cy.visit('http://cypress:password123@localhost:3500/timeout').then(() => {
        expect(backend).to.be.calledWith('resolve:url', 'http://localhost:3500/timeout')
      })
    })
    ...
    it('passes auth options', () => {
      ...
      cy.visit('http://localhost:3500/timeout', { auth }).then(() => {
      ...
      (done) => {
        cy.on('fail', (err) => {
          expect(err.message).to.contain(
            "`cy.visit()` failed because the 'file://...' protocol is not supported by Cypress."
          )
    ...
    it('can send querystring params', () => {
      ...
      cy.visit('http://localhost:3500/dump-qs', { qs }).then(() => {
    ...
    describe('location getter overrides', () => {
      before(() => {
        cy.visit('/fixtures/jquery.html?foo=bar#dashboard?baz=quux')
          .window()
          .as('win')
          .then((win) => {
    ...
    describe('.log', () => {
      ...
      it('indicates redirects in the message', () => {
        ...
        cy.visit('http://localhost:3500/foo').then(function () {
          ...
          expect(lastLog.get('message')).to.eq('http://localhost:3500/foo -> 1 -> 2')
        })
      })
      ...
      it('indicates POST in the message', () => {
        cy.visit('http://localhost:3500/post-only', {
          ...
          expect(lastLog.get('message')).to.eq('POST http://localhost:3500/post-only')
        })
      })
        ...
        cy.visit('http://localhost:3500/fixtures/generic.html#foo')
          ...
          .then(function () {
            expect(this.lastLog.invoke('consoleProps')).to.deep.eq({
              Command: 'visit',
              Note:
                'Because this visit was to the same hash, the page did not reload and the onBeforeLoad and onLoad callbacks did not fire.',
            })
          })
      })
    ...
    describe(
      ...
      () => {
        ...
        it('sets error command state', function (done) {
          cy.stub(Cypress, 'backend').withArgs('resolve:url').rejects(new Error())
        ...
        it('cancels resolve url promise on timeout', (done) => {
          ...
          cy.stub(Cypress, 'backend').withArgs('resolve:url').callsFake(fn)
        ...
        ;['foo', null, false].forEach((qs) => {
          const str = String(qs)

          it(`throws when qs is ${str}`, (done) => {
            cy.on('fail', (err) => {
              expect(err.message).to.contain(
                `\`cy.visit()\` requires the \`qs\` option to be an object, but received: \`${str}\``
              )
        ...
        it('throws attemping to visit 2 unique ip addresses', function (done) {
          ...
          cy.stub(Cypress, 'backend').withArgs('resolve:url').resolves({
            isOkStatusCode: true,
            isHtml: true,
          ...
          cy.stub(Cypress.utils, 'locExisting').returns(one)
          ...
          cy.visit('http://127.0.0.1:3500/fixtures/generic.html').visit('http://126.0.0.1:3500/fixtures/generic.html')
        })
        ...
        it('does not call resolve:url when throws attemping to visit a 2nd domain', (done) => {
          ...
          cy.visit('http://localhost:3500/fixtures/generic.html').visit('http://google.com:3500/fixtures/generic.html')
        })
        ...
        it('displays loading_network_failed when _resolveUrl throws', function (done) {
          ...
          cy.stub(Cypress, 'backend').withArgs('resolve:url').rejects(err1)
        ...
        it('displays loading_file_failed when _resolveUrl resp is not ok', function (done) {
          ...
          cy.stub(Cypress, 'backend').withArgs('resolve:url').resolves(obj)
        ...
        it('displays loading_file_failed redirects when _resolveUrl resp is not ok', function (done) {
          const obj = {
            ...
            status: 404,
            statusText: 'Not Found',
            redirects: ['301: http://localhost:3500/bar/'],
          }
        ...
        it('displays loading_http_failed when _resolveUrl resp is not ok', function (done) {
          ...
          cy.stub(Cypress, 'backend').withArgs('resolve:url', 'https://google.com/foo').resolves(obj)
        ...
        it('displays loading_http_failed redirects when _resolveUrl resp is not ok', function (done) {
          const obj = {
            ...
            redirects: ['302: https://google.com/bar/', '301: https://gmail.com/'],
          }
  ...
  context('#page load', () => {
    ...
    it('does not time out current commands until stability is reached', () => {
      ...
      cy.on('command:retry', (options) => {
        switch (options._retries) {
          case 1: {
            ...
            const $a = win.$("<a href='/timeout?ms=500'>jquery</a>").appendTo(win.document.body)
          ...
          case 2: {
            ...
            $("<div id='does-not-exist'>does not exist<div>").appendTo(win.document.body)
      ...
      cy.visit('/fixtures/jquery.html')
        ...
        .get('#does-not-exist', { timeout: 300 })
    ...
    describe('errors', () => {
      ...
      it('does time out once stability is reached', function (done) {
            ...
            case 1:
              {
                ...
                // to guarantee that url does not time out
                const $a = win.$("<a href='/timeout?ms=400'>jquery</a>").appendTo(win.document.body)
        ...
        cy.visit('/fixtures/jquery.html')
          ...
          .get('#does-not-exist', { timeout: 200 })
          .should('have.class', 'foo')
      })
      ...
      it('captures cross origin failures', function (done) {
        ...
        cy.visit('/fixtures/jquery.html')
          ...
          .then((win) => {
            ...
            const $a = win.$(`<a href='${url}'>jquery</a>`).appendTo(win.document.body)
  ...
  context('#page loading', () => {
    ...
    it('logs during form submission and yields stale element', () => {
      ...
      cy.visit('/fixtures/form.html').then(function () {
        ...
        cy.get('form#click-me')
          ...
          .then(function (subject) {
            ...
            expect(cy.queue.names()).to.deep.eq(names.concat(['visit', 'then', 'get', 'find', 'click', 'then']))
  ...
  context('#url:changed', () => {
    ...
    describe('page navigation', () => {
      it('emits url:changed event on initial visit', () => {
        ...
        cy.visit('/fixtures/generic.html').then(() => {
          expect(emit).to.be.calledOnce
          expect(emit).to.be.calledWith('url:changed', 'http://localhost:3500/fixtures/generic.html')
        })
      })
      ...
      it('emits url:changed on 2nd visit to different page', () => {
        ...
        cy.visit('/fixtures/generic.html')
          ...
          .then(() => {
            ...
            expect(emit).to.be.calledWith('url:changed', 'http://localhost:3500/fixtures/jquery.html')
          })
      })
      ...
      it('emits url:changed event on page navigation', () => {
        ...
        cy.visit('/fixtures/generic.html')
          ...
          .then(() => {
            ...
            expect(emit).to.be.calledWith('url:changed', 'http://localhost:3500/fixtures/dimensions.html')
          })
      })
      ...
      it('logs url:changed event on page navigation', () => {
        cy.visit('/fixtures/generic.html')
          ...
          .then(function () {
            ...
            expect(this.logs[0].get('message')).to.eq('http://localhost:3500/fixtures/dimensions.html')
    ...
    describe('hashchange events', () => {
      it('emits url:changed event', () => {
        ...
        cy.visit('/fixtures/generic.html')
          ...
          .click()
          .then(() => {
            expect(emit).to.be.calledWith('url:changed', 'http://localhost:3500/fixtures/generic.html#hashchange')
          })
      })
          ...
          .then((win) => {
            return new Promise((resolve) => {
              ...
              .then(() => {
                ...
                expect(emit.firstCall).to.be.calledWith('url:changed', 'http://localhost:3500/fixtures/generic.html')
                ...
                expect(emit.thirdCall).to.be.calledWith('url:changed', 'http://localhost:3500/fixtures/generic.html')
      ...
      it('logs url:changed event as navigation events occur', () => {
        cy.visit('/fixtures/generic.html')
          ...
          .then(function () {
            ...
            expect(this.logs[0].get('message')).to.eq('http://localhost:3500/fixtures/generic.html#hashchange')

            expect(this.logs[1].get('message')).to.eq('http://localhost:3500/fixtures/generic.html')

            expect(this.logs[2].get('message')).to.eq('http://localhost:3500/fixtures/generic.html#hashchange')
          })
      })
    })
      ...
      it('emits url:changed event', () => {
        ...
        cy.visit('/fixtures/generic.html')
          ...
          .then((win) => {
            ...
            expect(emit).to.be.calledWith('url:changed', 'http://localhost:3500/fixtures/pushState.html')
          })
      })
        ...
        cy.visit('/fixtures/generic.html')
          ...
          .then(function (win) {
            ...
            expect(lastLog.invoke('consoleProps')).to.deep.eq({
              ...
              Args: [{ foo: 'bar' }, null, 'pushState.html'],
            })
          })
      })
    })
      ...
      it('emits url:changed event', () => {
        ...
        cy.visit('/fixtures/generic.html')
          ...
          .then((win) => {
            ...
            expect(emit).to.be.calledWith('url:changed', 'http://localhost:3500/fixtures/replaceState.html')
          })
      })
        ...
        cy.visit('/fixtures/generic.html')
          ...
          .then(function (win) {
            ...
            expect(lastLog.invoke('consoleProps')).to.deep.eq({
              ...
              Args: [{ foo: 'bar' }, null, 'replaceState.html'],
            })
          })
      })
    })
  ...
  context('#form sub', () => {
    ...
    it("logs 'form sub'", () => {
      ...
      cy.visit('/fixtures/form.html').then(() => {
        ...
        cy.get('#click-me')
          ...
          .then(function () {
            ...
            expect(this.logs[0].get('message')).to.eq('--submitting form--')
