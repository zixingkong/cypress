      ...
      describe(
        ...
        () => {
          it('throws when options.length isnt a number', (done) => {
            cy.on('fail', (err) => {
              expect(err.message).to.include(
                'You must provide a valid number to a `length` assertion. You passed: `asdf`'
              )
          ...
          it('throws on too many elements after timing out waiting for length', (done) => {
            ...
            cy.on('fail', (err) => {
              expect(err.message).to.include(
                `Too many elements found. Found '${el.length}', expected '${el.length - 1}'.`
              )
          ...
          it('throws on too few elements after timing out waiting for length', (done) => {
            ...
            cy.on('fail', (err) => {
              expect(err.message).to.include(
                `Not enough elements found. Found '${el.length}', expected '${el.length + 1}'.`
              )
          ...
          it('returns no elements', (done) => {
            const errIncludes = (el, node) => {
              ...
              cy.on('fail', (err) => {
                expect(err.message).to.include(
                  `Expected to find element: \`${el}\`, but never found it. Queried from element: ${node}`
                )
  ...
  describe(
    ...
    () => {
      ...
      it('errors after timing out not finding element', (done) => {
        cy.on('fail', (err) => {
          expect(err.message).to.include(
            'Expected to find element: `span`, but never found it. Queried from element: <li.item>'
          )
