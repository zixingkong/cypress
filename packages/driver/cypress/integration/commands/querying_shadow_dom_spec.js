...
describe('src/cy/commands/querying - shadow dom', () => {
  ...
  context('#within', () => {
    it('finds element within shadow dom with includeShadowDom option', () => {
      cy.get('#parent-of-shadow-container-0').within(() => {
        cy.get('p', { includeShadowDom: true }).should('have.length', 1).should('have.text', 'Shadow Content 3')
      })
    })
    ...
    it('when within subject is shadow root, finds element without needing includeShadowDom option', () => {
      cy.get('#shadow-element-1')
        .shadow()
        .within(() => {
          cy.get('p').should('have.length', 1).should('have.text', 'Shadow Content 1')
        })
    })
    ...
    it('when within subject is already in shadow dom, finds element without needing includeShadowDom option', () => {
      cy.get('.shadow-8-nested-1', { includeShadowDom: true }).within(() => {
        cy.get('.shadow-8-nested-5').should('have.text', '8')
      })
    })
  })
  ...
  context('#get', () => {
    it('finds elements within shadow roots', () => {
      cy.get('.shadow-1', { includeShadowDom: true }).should('have.text', 'Shadow Content 1')
    })

    it('finds shadow elements within shadow roots', () => {
      cy.get('.shadow-5', { includeShadowDom: true }).should('have.text', 'Shadow Content 5')
    })

    it('finds light elements within shadow slots', () => {
      cy.get('.in-shadow-slot', { includeShadowDom: true }).should('have.text', 'In Shadow Slot')
    })
    ...
    it.skip('finds elements within shadow roots with cross-boundary selector', () => {
      cy.get('#parent-of-shadow-container-0 .shadow-3', {
        includeShadowDom: true,
      }).should('have.text', 'Shadow Content 3')
    })

    it('finds elements outside shadow roots', () => {
      cy.get('#non-shadow-element', { includeShadowDom: true }).should('have.text', 'Non Shadow')
    })

    it('finds elements in and out of shadow roots', () => {
      cy.get('.in-and-out', { includeShadowDom: true }).should('have.length', 2)
    })
  ...
  context('#contains', () => {
    it('finds element within shadow dom based on text content', () => {
      cy.contains('Shadow Content 1', { includeShadowDom: true }).should('have.class', 'shadow-1')
    })

    it('finds element within shadow dom based on text content, scoped to selector', () => {
      cy.contains('.shadow-11-scope', 'Find Me With cy.contains', {
        includeShadowDom: true,
      }).should('have.length', 1)
    })

    it('finds element within shadow dom based when already past shadow root', () => {
      cy.get('#shadow-element-1').shadow().contains('Shadow Content 1').should('have.class', 'shadow-1')
    })
  })

  context('#shadow', () => {
    it('returns an empty set if no shadow roots exist', () => {
      cy.get('#non-shadow-element')
        .shadow()
        .should('not.exist')
        .then(($root) => {
    ...
    it('has a custom error message if it cannot find a root', (done) => {
      cy.on('fail', (err) => {
        expect(err.message).to.equal(
          `Timed out retrying after 0ms: Expected the subject to host a shadow root, but never found it.`
        )
