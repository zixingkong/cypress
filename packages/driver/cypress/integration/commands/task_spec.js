        ...
        () => {
          ...
          it('throws when task is absent', function (done) {
            cy.on('fail', (err) => {
              ...
              expect(err.message).to.eq(
                '`cy.task()` must be passed a non-empty string as its 1st argument. You passed: ``.'
              )
          ...
          it("throws when task isn't a string", function (done) {
            cy.on('fail', (err) => {
              ...
              expect(lastLog.get('state')).to.eq('failed')
              expect(err.message).to.eq(
                '`cy.task()` must be passed a non-empty string as its 1st argument. You passed: `3`.'
              )
              expect(err.docsUrl).to.eq('https://on.cypress.io/task')
