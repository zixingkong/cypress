        ...
        () => {
          ...
          it('throws when cmd is absent', function (done) {
            cy.on('fail', (err) => {
              ...
              expect(err.message).to.eq(
                "`cy.exec()` must be passed a non-empty string as its 1st argument. You passed: ''."
              )
          ...
          it("throws when cmd isn't a string", function (done) {
            cy.on('fail', (err) => {
              ...
              expect(lastLog.get('state')).to.eq('failed')
              expect(err.message).to.eq(
                "`cy.exec()` must be passed a non-empty string as its 1st argument. You passed: '3'."
              )
              expect(err.docsUrl).to.eq('https://on.cypress.io/exec')
          ...
          describe('when error code is non-zero', () => {
            it('throws error that includes useful information and exit code', (done) => {
              ...
              cy.on('fail', (err) => {
                expect(err.message).to.contain(
                  "`cy.exec('ls')` failed because the command exited with a non-zero code.\n\nPass `{failOnNonZeroExit: false}` to ignore exit code failures."
                )
            ...
            describe('and failOnNonZeroExit is false', () => {
              it('does not error', () => {
                ...
                cy.exec('ls', { failOnNonZeroExit: false }).should('deep.eq', response)
              })
