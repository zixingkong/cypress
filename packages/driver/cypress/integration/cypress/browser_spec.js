...
describe('src/cypress/browser', () => {
  ...
  context('.browser', () => {
    it('returns the current browser', function () {
      expect(this.commands().browser).to.eql({
        name: 'chrome',
        family: 'chromium',
      })
    })
    ...
    it('throws if arg is not a string or object', function () {
      expect(() => {
        this.commands()
          .isBrowser(true)
          .to.throw(
            '`Cypress.isBrowser()` must be passed the name of a browser, an object to filter with, or an array of either. You passed: `true`'
          )
      })
    })
