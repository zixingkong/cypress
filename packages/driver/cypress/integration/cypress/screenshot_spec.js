...
describe('src/cypress/screenshot', () => {
  ...
  context('.defaults', () => {
    ...
    it('sets clip if specified', () => {
      ...
      expect(Screenshot.getConfig().clip).to.eql({
        width: 200,
        height: 100,
        x: 0,
        y: 0,
    ...
    describe('errors', () => {
      ...
      it('throws if padding is not a number or an array of numbers with a length between 1 and 4', () => {
        expect(() => {
          ...
          '`Cypress.Screenshot.defaults()` `padding` option must be either a number or an array of numbers with a maximum length of 4. You passed: `50px`'
        )

        expect(() => {
          Screenshot.defaults({ padding: ['bad', 'bad', 'bad', 'bad'] })
        }).to.throw(
          '`Cypress.Screenshot.defaults()` `padding` option must be either a number or an array of numbers with a maximum length of 4. You passed: `bad, bad, bad, bad`'
        )

        expect(() => {
          Screenshot.defaults({ padding: [20, 10, 20, 10, 50] })
        }).to.throw(
          '`Cypress.Screenshot.defaults()` `padding` option must be either a number or an array of numbers with a maximum length of 4. You passed: `20, 10, 20, 10, 50`'
        )
      })
      ...
      it('throws if clip is lacking proper keys', () => {
        expect(() => {
          ...
          '`Cypress.Screenshot.defaults()` `clip` option must be an object with the keys `{ width, height, x, y }` and number values. You passed: `{x: 5}`'
        )
      })
      ...
      it('throws if clip has extraneous keys', () => {
        expect(() => {
          ...
          '`Cypress.Screenshot.defaults()` `clip` option must be an object with the keys `{ width, height, x, y }` and number values. You passed: `Object{5}`'
        )
      })
      ...
      it('throws if clip has non-number values', () => {
        expect(() => {
          Screenshot.defaults({
            clip: { width: 100, height: 100, x: 5, y: '5' },
          })
        }).to.throw(
          '`Cypress.Screenshot.defaults()` `clip` option must be an object with the keys `{ width, height, x, y }` and number values. You passed: `Object{4}`'
        )
      })
