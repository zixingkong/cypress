  ...
  return Commands.addAll(
    { prevSubject: ['element', 'window'] },
    {
      focus(subject, options = {}) {
        ...
        const isBody =
          $dom.isJquery(options.$el) && $elements.isElement(options.$el.get(0)) && $elements.isBody(options.$el.get(0))
      ...
      blur(subject, options = {}) {
        ...
        if (options.force !== true && !$focused && !isWin && !isBody) {
          ...
          $errUtils.throwErrByPath('blur.no_focused_element', {
            onFail: options._log,
          })
        }
        ...
        // as $focused
        if (options.force !== true && !isWin && !isBody && options.$el.get(0) !== $focused.get(0)) {
          if (options.error === false) {
            return
          }
