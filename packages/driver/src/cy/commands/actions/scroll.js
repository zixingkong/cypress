    ...
    { prevSubject: 'element' },
    {
      scrollIntoView(subject, options = {}) {
        ...
        if (!_.isObject(userOptions)) {
          $errUtils.throwErrByPath('scrollIntoView.invalid_argument', {
            args: { arg: userOptions },
          })
        }
        ...
        if (subject.length > 1) {
          $errUtils.throwErrByPath('scrollIntoView.multiple_elements', {
        ...
        if (isNaNOrInfinity(options.duration)) {
          $errUtils.throwErrByPath('scrollIntoView.invalid_duration', {
            args: { duration: options.duration },
          })
        }

        if (!(options.easing === 'swing' || options.easing === 'linear')) {
          $errUtils.throwErrByPath('scrollIntoView.invalid_easing', {
            args: { easing: options.easing },
          })
        }
    ...
    { prevSubject: ['optional', 'element', 'window'] },
    {
      scrollTo(subject, xOrPosition, yOrOptions, options = {}) {
        ...
        if (!isWin && $container.length > 1) {
          $errUtils.throwErrByPath('scrollTo.multiple_containers', {
            args: { num: $container.length },
          })
        }
        ...
        // which could be 500 or "500", then it's NaN...throw
        if (isNaNOrInfinity(options.duration)) {
          $errUtils.throwErrByPath('scrollTo.invalid_duration', {
            args: { duration: options.duration },
          })
        }

        if (!(options.easing === 'swing' || options.easing === 'linear')) {
          $errUtils.throwErrByPath('scrollTo.invalid_easing', {
            args: { easing: options.easing },
          })
        }

        if (!_.isBoolean(options.ensureScrollable)) {
          $errUtils.throwErrByPath('scrollTo.invalid_ensureScrollable', {
            args: { ensureScrollable: options.ensureScrollable },
          })
        }
        ...
        if (isNaNOrInfinity(options.y) || isNaNOrInfinity(options.x)) {
          $errUtils.throwErrByPath('scrollTo.invalid_target', {
            args: { x, y },
          })
        }

        if (options.log) {
          const deltaOptions = $utils.stringify($utils.filterOutOptions(options, { duration: 0, easing: 'swing' }))
