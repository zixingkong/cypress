    ...
    { prevSubject: 'element' },
    {
      select(subject, valueOrText, options = {}) {
        ...
        if (!options.$el.is('select')) {
          ...
          $errUtils.throwErrByPath('select.invalid_element', {
            args: { node },
          })
        }

        if (options.$el.length && options.$el.length > 1) {
          $errUtils.throwErrByPath('select.multiple_elements', {
        ...
        const retryOptions = () => {
          return Promise.try(getOptions).catch((err) => {
            options.error = err
        ...
        return Promise.try(retryOptions).then((obj = {}) => {
