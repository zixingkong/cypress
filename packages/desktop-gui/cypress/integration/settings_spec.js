...
describe('Settings', () => {
  ...
  const openConfiguration = () => {
    ...
    cy.get('.config-vars').should('be.visible').invoke('height').should('be.gt', 400)
  }
  ...
  describe('configuration panel', () => {
    describe('displays config', () => {
      ...
      it('removes the summary list of values once a key is expanded', () => {
        cy.contains('span', 'browsers').parents('div').first().find('span').first().click()
        cy.get('.config-vars').invoke('text').should('not.contain', 'Chrome, Chromium')

        cy.get('.config-vars').invoke('text').should('contain', '0:Chrome')
      })

      it('distinguishes between Arrays and Objects when expanded', () => {
        cy.get('.config-vars').invoke('text').should('not.contain', 'browsers: Array (4)')

        cy.contains('span', 'browsers').parents('div').first().find('span').first().click()
        cy.get('.config-vars').invoke('text').should('contain', 'browsers: Array (4)')
      })
      ...
      it('applies the same color treatment to expanded key values as the root key', () => {
        ...
        cy.get('@config-vars').contains('span', 'Canary').parent('span').should('have.class', 'plugin')

        cy.get('@config-vars').contains('span', 'Electron').parent('span').should('have.class', 'plugin')
        ...
        cy.get('@config-vars').contains('span', '127.0.0.1').parent('span').should('have.class', 'config')

        cy.get('@config-vars').contains('span', '127.0.0.2').parent('span').should('have.class', 'config')
      ...
      it('displays string values as quoted strings', () => {
        cy.get('.config-vars').invoke('text').should('contain', 'baseUrl:"http://localhost:8080"')
      })

      it('displays undefined and null without quotations', () => {
        cy.get('.config-vars').invoke('text').should('not.contain', '"undefined"').should('not.contain', '"null"')
      })
      ...
      it('opens help link on click', () => {
        cy.get('.settings-config .learn-more')
          .click()
          .then(function () {
            expect(this.ipc.externalOpen).to.be.calledWithMatch({
      ...
      it('displays env settings', () => {
        cy.get('@config').then(({ resolved }) => {
          const getEnvKeys = flow([get('env'), toPairs, map(([key]) => key), sortBy(get(''))])
          ...
          const assertFromTooltipsExist = flow([
            map((key) => {
              return [key, flow([get(['env', key, 'from']), (from) => `.${from}`])(resolved)]
            }),
    ...
    describe('experimental feature exists', () => {
      ...
      context('enabled', () => {
        ...
        it('displays experiment', () => {
          ...
          cy.get('.experiment-status-sign').should('have.class', 'enabled').and('have.text', 'enabled')
      ...
      context('disabled', () => {
        ...
        it('displays experiment', () => {
          cy.get('.settings-experiments').contains('Cool Feature')
          cy.get('.experiment-status-sign').should('have.class', 'disabled').and('have.text', 'disabled')
  ...
  describe('file preference panel', () => {
    ...
    describe('when editors load with preferred editor', () => {
      ...
      it('sets editor through ipc when a different editor is selected', function () {
        cy.contains('Atom').click().closest('li').should('have.class', 'is-selected')
