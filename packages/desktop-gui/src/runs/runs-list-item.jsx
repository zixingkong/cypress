...
const RunDuration = ({ run }) => {
  ...
  if (run.totalDuration) {
      ...
      return (
        <Tooltip title="Parallelization was disabled for this run." placement="top" className="cy-tooltip">
          <span className="env-duration">
            <i className="fas fa-exclamation-triangle orange"></i> {durationFormatted(run.totalDuration)}
          </span>
        </Tooltip>
      )
    }
    ...
    return (
      <span className="env-duration">
        <i className="fas fa-hourglass-end"></i> {durationFormatted(run.totalDuration)}
      </span>
    )
  }
  ...
  // Run is still going
  if (run.createdAt) {
    return <TimerDisplay startTime={run.createdAt} />
  }
...
export default class RunsListItem extends Component {
  render() {
    ...
    return (
      <li onClick={this._goToRun}>
            ...
            {run.commit ? (
              <div className="msg">
                {run.commit.authorEmail ? (
                  <img className="user-avatar" height="13" width="13" src={`${gravatarUrl(run.commit.authorEmail)}`} />
                ) : null}
                {run.commit.message ? (
                  <span className="commit-msg">{run.commit.message.split(NEWLINE)[0]}</span>
                ) : (
                  <div className="msg italic">- No commit info found -</div>
                )}
              </div>
            ) : (
              <div className="msg italic">- No commit info found -</div>
        ...
        <div className="row-column-wrapper">
          <div>
            <i className="far fa-clock"></i> {getFormattedTimeFromNow(run.createdAt)}
          </div>
        </div>
        <div className="row-column-wrapper">
          ...
          <div className="td-env-padding">
            ...
            {this._instancesExist() ? (
              this._moreThanOneInstance() && this._osLength() > 1 ? (
                <div>
                  <i className="fas fa-fw fa-desktop"></i> {this._osLength()} OSs
                </div>
              ) : (
                // or did we only actual run it on one OS
                <div>
                  <i className={`fa-fw ${this._osIcon()}`}></i> {this._osDisplay()}
                </div>
              )
            ) : null}
            ...
            {this._instancesExist() ? (
              this._moreThanOneInstance() && this._browsersLength() > 1 ? (
                <div className="env-msg">
                  <i className="fas fa-fw fa-globe"></i> {this._browsersLength()} browsers
                </div>
              ) : (
                // or did we only actual run it on one browser
                <div className="env-msg">
                  {this._browserIcon()} {this._browserDisplay()}
                </div>
              )
            ) : null}
          </div>
        ...
        <div className="row-column-wrapper passing-data">
          {run.status !== 'running' ? (
            <div className="result">
              <i className="fas fa-check"></i> <span>{run.totalPassed || '0'}</span>
            </div>
          ) : null}
        </div>
        <div className="row-column-wrapper failure-data">
          {run.status !== 'running' ? (
            <div className="result">
              <i className="fas fa-times"></i> <span>{run.totalFailed || '0'}</span>
            </div>
          ) : null}
        </div>
      </li>
  ...
  _displaySpec() {
    if (this.props.run.instances.length === 1) {
      return this.props.run.instances[0].spec ? (
        <span>
          {this.props.run.instances[0].integrationFolder
            ? stripSharedDirsFromDir2(
                this.props.run.instances[0].integrationFolder,
                this.props.run.instances[0].spec,
                this.props.run.instances[0].platform.osName
              )
              ? stripSharedDirsFromDir2(
                  this.props.run.instances[0].integrationFolder,
                  this.props.run.instances[0].spec,
  ...
  _osDisplay = () => {
    if (this.props.run.instances && this.props.run.instances[0]) {
      return `${_.get(
        this.props.run,
        'instances[0].platform.osVersionFormatted',
        this.props.run.instances[0].osFormatted
      )}`
    }
  }
  ...
  _browserDisplay = () => {
    if (this.props.run.instances && this.props.run.instances[0]) {
      return `${browserNameFormatted(
        _.get(this.props.run, 'instances[0].platform.browserName', '')
      )} ${browserVersionFormatted(_.get(this.props.run, 'instances[0].platform.browserVersion', ''))}`
    }
  }
