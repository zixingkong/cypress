...
const fixMouseOut = (fn, getTarget) => (e) => {
  if (!e.relatedTarget || e.relatedTarget.parentNode === getTarget() || e.relatedTarget === getTarget()) return
          ...
          <div className="wrap" onMouseOver={this._setHighlight(true)}>
            ...
            <Tooltip title={model.infoHelp || ''} className="cy-tooltip">
              <span className="info num-elements">
                {model.isValid ? model.numElements : <i className="fas fa-exclamation-triangle"></i>}
              </span>
            </Tooltip>
          </div>
    ...
    return (
      ...
      >
        <button onClick={this._toggleMethodPicker}>
          <i className="fas fa-caret-down"></i> cy.{model.method}
        </button>
