...
const { runIsolatedCypress, snapshotMochaEvents, getAutCypress } = helpers.createCypress({
  config: { retries: 2, isTextTerminal: true, firefoxGcInterval: null },
})
const { sinon } = Cypress
...
const threeTestsWithRetry = {
  suites: {
    'suite 1': {
      ...
      tests: ['test 1', { name: 'test 2', fail: 2 }, 'test 3'],
    },
  },
}
...
describe('src/cypress/runner retries mochaEvents', { retries: 0 }, () => {
  ...
  it('can set retry config', () => {
    runIsolatedCypress({}, { config: { retries: 1 } }).then(({ autCypress }) => {
    ...
    runIsolatedCypress(
      {
        ...
        suites: {
          'suite 1': {
            hooks: ['before', 'beforeEach', 'beforeEach', 'afterEach', 'after'],
  ...
  describe('screenshots', () => {
    it('retry screenshot in test body', () => {
      ...
      runIsolatedCypress(
        ...
        expect(autCypress.automation.withArgs('take:screenshot').args[0]).matchSnapshot({
          startTime: match.string,
          testAttemptIndex: match(0),
        })
        expect(onAfterScreenshot.args[0][0]).to.matchSnapshot({
          testAttemptIndex: match(0),
        })
        expect(onAfterScreenshot.args[2][0]).to.matchDeep({
          testAttemptIndex: 1,
        })
      })
    })
