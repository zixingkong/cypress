      ...
      <div className="command-wrapper">
        <div className="command-wrapper-text">
          <span className="command-message">
            <span className="command-message-text">Interact with your site to add test commands.</span>
          </span>
...
const Hook = observer(({ model, showNumber }: HookProps) => (
  <li
    className={cs('hook-item', {
      'hook-failed': model.failed,
      'hook-studio': model.isStudio,
    })}
  >
    <Collapsible
    ...
    >
      <ul className="commands-container">
        {_.map(model.commands, (command) => (
          <Command key={command.id} model={command} aliasesWithDuplicates={model.aliasesWithDuplicates} />
        ))}
