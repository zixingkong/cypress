describe('redirects + requests', () => {
  ...
  it('automatically follows redirects', () => {
    cy.request('http://localhost:2294/redirect').then((resp) => {
  ...
  it('sets Accept header to */* by default', () => {
    cy.request('http://localhost:2294/headers').its('body').its('headers').its('accept').should('eq', '*/*')
  })
  ...
  it('can make requests that take more than `responseTimeout` to complete', () => {
    return Cypress.$.get(`http://localhost:2290/timeout?ms=${Cypress.config('responseTimeout') * 2}`).then((data) => {
