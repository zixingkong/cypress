...
describe('lib/settings', () => {
  context('with no configFile option', () => {
    ...
    context('.readEnv', () => {
      ...
      it('parses json', () => {
        ...
        return settings.readEnv(projectRoot).then((obj) => {
      ...
      it('throws when invalid json', () => {
        ...
        return settings.readEnv(projectRoot).catch((err) => {
  ...
  context('with configFile: false', () => {
    ...
    it('.exists passes', function () {
      return settings.exists(this.projectRoot, this.options).then((exists) => {
        expect(exists).to.equal(undefined)
      })
    })

    it('.write does not create a file', function () {
      return settings.write(this.projectRoot, {}, this.options).then(() => {
        return fs.exists(path.join(this.projectRoot, 'cypress.json')).then((exists) => {
    ...
    it('.read returns empty object', function () {
      return settings.read(this.projectRoot, this.options).then((settings) => {
  ...
  context('with a configFile set', () => {
    ...
    it(".exists fails when configFile doesn't exist", function () {
      return settings.exists(this.projectRoot, this.options).catch((error) => {
        expect(error.type).to.equal('CONFIG_FILE_NOT_FOUND')
      })
    })

    it('.write creates configFile', function () {
      return settings.write(this.projectRoot, { foo: 'bar' }, this.options).then(() => {
        return fs.readJsonAsync(path.join(this.projectRoot, this.options.configFile)).then((json) => {
    ...
    it('.read returns from configFile', function () {
      return fs
        .writeJsonAsync(path.join(this.projectRoot, this.options.configFile), {
          foo: 'bar',
        })
        .then(() => {
          return settings.read(this.projectRoot, this.options).then((settings) => {
