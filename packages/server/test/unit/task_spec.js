...
describe('lib/task', () => {
  ...
  it("throws if 'task' event resolves __cypress_unhandled__", function () {
    ...
    return task
      ...
      .catch((err) => {
        expect(err.message).to.equal(
          `The task 'some:task' was not handled in the plugins file. The following tasks are registered: foo, bar\n\nFix this in your plugins file here:\n${this.pluginsFile}\n\nhttps://on.cypress.io/api/task`
        )
      })
  })
  ...
  it("throws if 'task' event resolves undefined", function () {
    ...
    return task
      ...
      .catch((err) => {
        expect(err.message).to.equal(
          `The task 'some:task' returned undefined. You must return a value, null, or a promise that resolves to a value or null to indicate that the task was handled.\n\nThe task handler was:\n\nfunction () {}\n\nFix this in your plugins file here:\n${this.pluginsFile}\n\nhttps://on.cypress.io/api/task`
        )
      })
  })
