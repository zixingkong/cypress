...
context('lib/browsers/cdp_automation', () => {
  ...
  context('.CdpAutomation', () => {
    beforeEach(function () {
      ...
      this.sendDebuggerCommand.throws(new Error('not stubbed')).withArgs('Browser.getVersion').resolves()
    ...
    describe('get:cookies', () => {
      beforeEach(function () {
        this.sendDebuggerCommand.withArgs('Network.getAllCookies').resolves({
          cookies: [
            {
              name: 'foo',
              value: 'f',
      ...
      it('returns all cookies', function () {
        return this.onRequest('get:cookies', { domain: 'localhost' }).then((resp) => {
    ...
    describe('get:cookie', () => {
      ...
      it('returns a specific cookie by name', function () {
        return this.onRequest('get:cookie', {
          domain: 'google.com',
          name: 'session',
        }).then((resp) => {
          expect(resp).to.deep.eq({
    ...
    describe('set:cookie', () => {
      ...
      it('resolves with the cookie props', function () {
        return this.onRequest('set:cookie', {
          domain: 'google.com',
          name: 'session',
          value: 'key',
          path: '/',
    ...
    describe('clear:cookie', () => {
      ...
      it('resolves single removed cookie', function () {
        return this.onRequest('clear:cookie', {
          domain: 'google.com',
          name: 'session',
        }).then((resp) => {
          expect(resp).to.deep.eq({
    ...
    describe('take:screenshot', () => {
      it('resolves with base64 data URL', function () {
        ...
        return expect(this.onRequest('take:screenshot')).to.eventually.equal('data:image/png;base64,foo')
      })
      ...
      it('rejects nicely if Page.captureScreenshot fails', function () {
        ...
        return expect(this.onRequest('take:screenshot')).to.be.rejectedWith(
