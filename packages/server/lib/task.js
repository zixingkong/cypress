  ...
  run(pluginsFilePath, options) {
    ...
    return Promise.try(() => {
      if (!plugins.has('task')) {
        ...
        throwKnownError(
          `The 'task' event has not been registered in the plugins file. You must register it before using cy.task()${fileAndDocsUrl}`
        )
      }
      ...
      .then((result) => {
        if (result === '__cypress_unhandled__') {
          ...
          return plugins.execute('_get:task:keys').then((keys) => {
            return throwKnownError(
              `The task '${
                options.task
              }' was not handled in the plugins file. The following tasks are registered: ${keys.join(
                ', '
              )}${fileAndDocsUrl}`
            )
          })
        ...
        if (result === undefined) {
          ...
          return plugins.execute('_get:task:body', options.task).then((body) => {
            ...
            return throwKnownError(
              `The task '${options.task}' returned undefined. You must return a value, null, or a promise that resolves to a value or null to indicate that the task was handled.${handler}${fileAndDocsUrl}`
            )
          })
        }
