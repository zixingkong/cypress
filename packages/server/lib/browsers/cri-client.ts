...
namespace CRI {
  ...
  export type EventName = 'Page.screencastFrame' | 'Page.downloadWillBegin' | 'Page.downloadProgress'
}
...
export const create = Bluebird.method(
  (target: websocketUrl, onAsynchronousError: Function): Bluebird<CRIWrapper> => {
    ...
    const reconnect = () => {
      ...
      return connect()
        .then(() => {
          debug('restoring subscriptions + running queued commands... %o', {
            subscriptions,
            enqueuedCommands,
          })
    ...
    return connect().then(() => {
      const ensureMinimumProtocolVersion = (protocolVersion: string) => {
        return getProtocolVersion().then((actual) => {
      ...
      client = {
        ...
        send: Bluebird.method((command: CRI.Command, params?: object) => {
          ...
          if (connected) {
            return cri.send(command, params).catch((err) => {
              if (!WEBSOCKET_NOT_OPEN_RE.test(err.message)) {
                throw err
              }

              debug('encountered closed websocket on send %o', {
                command,
                params,
                err,
              })
