...
const _debugWorker = !debug.enabled
  ...
  : (worker: WorkerInfo) => {
      return {
        ..._.pick(worker, 'isBusy', 'id'),
        freeWorkers: _.filter(workers, { isBusy: false }).length,
      }
    }
...
async function sendRewrite(worker: WorkerInfo, opts: RewriteOpts): Promise<string> {
  ...
  debug('sending rewrite to worker %o', {
    worker: _debugWorker(worker),
...
export function queueRewriting(opts: RewriteOpts): Promise<string> {
  ...
  // otherwise enqueue
  debug('enqueuing source for rewriting %o', {
    opts: _debugOpts(opts),
    prevQueueLength: queued.length,
  })
